...
# Env: VPS_SSH=user@host (required), VPS_DIR=~/kappa-bot (default),
#   DOMAIN (default kappa.shamah.dev), SERVICE_PORT (default 3443).
set -euo pipefail

VPS_SSH="${VPS_SSH:?set VPS_SSH=user@host}"
VPS_DIR="${VPS_DIR:-~/kappa-bot}"
DOMAIN="${DOMAIN:-kappa.shamah.dev}"
SERVICE_PORT="${SERVICE_PORT:-3443}"
...
ssh "$VPS_SSH" bash -s -- "$SHA" "$VPS_DIR" "$DOMAIN" "$SERVICE_PORT" <<'REMOTE'
set -euo pipefail
SHA="$1"; DIR="${2/#\~/$HOME}"; DOMAIN="$3"; SERVICE_PORT="$4"
cd "$DIR"
...
# 6. health (service direct on localhost; web + api routing through nginx)
curl -sf "http://127.0.0.1:$SERVICE_PORT/health" >/dev/null
...
echo "health: service :$SERVICE_PORT/health 200, / 200, /api/v1/auth/me 401"
