...
export async function createApp(): Promise<Gateway> {
  ...
  return { config, log, client, registry, jobs, close };
}

/** Full gateway start: migrate → REST → login → cron (no HTTP listener; service owns /health). */
export async function startGateway(): Promise<Gateway> {
  const app = await createApp();
  const { config, log, client, jobs } = app;
